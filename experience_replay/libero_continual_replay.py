...
REPLAY_CHILD_FLAG = "--replay-train-child"
...
class Config:
    ...
    replay_transitions_per_task: int
    replay_seed: int
    replay_strict_counts: bool
    cudnn_benchmark: str
...
def load_config() -> Config:
    ...
    replay_transitions = parse_int("REPLAY_TRANSITIONS", 0)
    if replay_transitions < 0:
        raise ConfigError("REPLAY_TRANSITIONS must be greater than or equal to 0")
    replay_transitions_per_task = parse_int("REPLAY_TRANSITIONS_PER_TASK", 1000)
    if replay_transitions_per_task < 0:
        raise ConfigError("REPLAY_TRANSITIONS_PER_TASK must be greater than or equal to 0")
    ...
    return Config(
        ...
        replay_root=Path(env("REPLAY_ROOT", str(run_output_root / "replay"))).expanduser(),
        ...
        replay_transitions_per_task=replay_transitions_per_task,
        replay_seed=parse_int("REPLAY_SEED", 0),
        replay_strict_counts=parse_bool("REPLAY_STRICT_COUNTS", False),
...
def safe_config_for_results(config: Config) -> dict[str, Any]:
    return {
        ...
        "replay_transitions": config.replay_transitions,
        "replay_transitions_per_task": config.replay_transitions_per_task,
        "replay_seed": config.replay_seed,
        "replay_strict_counts": config.replay_strict_counts,
...
def read_frame_indices_by_task(root: Path, task_ids: list[int]) -> dict[int, list[int]]:
    """Return absolute LeRobot frame indices grouped by task_index."""

    data_root = root / "data"
    if not data_root.exists():
        raise FileNotFoundError(f"Dataset data directory does not exist: {data_root}")
    ...
    indices_by_task = {int(task_id): [] for task_id in task_ids}

    for parquet_file in parquet_files:
        frame_data = pd.read_parquet(parquet_file, columns=["index", "task_index"])
        selected = frame_data[frame_data["task_index"].isin(task_id_set)]
        if selected.empty:
            continue

        for task_id, task_rows in selected.groupby("task_index", sort=False):
            indices_by_task[int(task_id)].extend(as_int(index) for index in task_rows["index"].tolist())

    return indices_by_task


def empty_indices_by_task(task_ids: list[int]) -> dict[int, list[int]]:
    return {int(task_id): [] for task_id in task_ids}


def load_indices_for_manifest(config: Config, suite: str) -> dict[int, list[int]]:
    root = require_dataset_root(config, suite)
    if config.dry_run and not root.exists():
        print(f"DRY_RUN enabled; skipping frame-index scan for missing dataset root: {root}")
        return empty_indices_by_task(config.train_task_ids)
    try:
        return read_frame_indices_by_task(root, config.train_task_ids)
    except ModuleNotFoundError as exc:
        if config.dry_run:
            print(
                "DRY_RUN enabled; skipping frame-index scan because an optional "
                f"dependency is missing: {exc.name}"
            )
            return empty_indices_by_task(config.train_task_ids)
        raise


def replay_enabled(config: Config) -> bool:
    return config.replay_transitions_per_task > 0 or config.replay_transitions > 0


def replay_budget_mode(config: Config) -> str:
    return "per_task" if config.replay_transitions_per_task > 0 else "fixed_total"
...
def allocate_replay_counts(
    ...
    pairs = [(suite, int(task_id)) for suite in previous_suites for task_id in task_ids]
    if not pairs:
        return {pair: 0 for pair in pairs}

    if config.replay_transitions_per_task > 0:
        return {pair: config.replay_transitions_per_task for pair in pairs}

    if config.replay_transitions <= 0:
        return {pair: 0 for pair in pairs}

    base_count, remainder = divmod(config.replay_transitions, len(pairs))
    return {
        pair: base_count + (1 if index < remainder else 0)
...
def build_replay_manifest(
    ...
    suite: str,
    previous_suites: list[str],
) -> ReplayManifestResult:
    manifest_path = config.replay_root / f"train_{suite}_replay_manifest.json"
    current_root = require_dataset_root(config, suite)
    current_indices_by_task = load_indices_for_manifest(config, suite)
    current_task_counts = {
        str(task_id): len(indices)
        for task_id, indices in current_indices_by_task.items()
    }

    replay_counts = allocate_replay_counts(config, previous_suites, config.train_task_ids)
    rng = random.Random(config.replay_seed)
    replay_sources: list[dict[str, Any]] = []

    for previous_suite in previous_suites:
        previous_root = require_dataset_root(config, previous_suite)
        previous_indices_by_task = load_indices_for_manifest(config, previous_suite)

        for task_id in config.train_task_ids:
            requested_count = replay_counts.get((previous_suite, int(task_id)), 0)
            available_indices = previous_indices_by_task[int(task_id)]
            actual_count = min(requested_count, len(available_indices))

            if requested_count > len(available_indices):
                message = (
                    f"Replay request for {previous_suite} task {task_id} asked for "
                    f"{requested_count} transitions but only {len(available_indices)} are available."
                )
                if config.replay_strict_counts and not config.dry_run:
                    raise RuntimeError(message)
                print("Warning: " + message)

            selected_indices = (
                rng.sample(available_indices, actual_count)
                if actual_count > 0
                else []
            )
            ...
            replay_sources.append(
                {
                    "suite": previous_suite,
                    "repo_id": config.dataset_repo_ids[previous_suite],
    ...
    replay_requested = sum(source["requested_count"] for source in replay_sources)
    replay_selected = sum(source["selected_count"] for source in replay_sources)
    current_selected = sum(current_task_counts.values())
    ...
    payload: dict[str, Any] = {
        ...
        "current": {
            ...
            "task_ids": [int(task_id) for task_id in config.train_task_ids],
            "task_counts": current_task_counts,
            "selected_count": current_selected,
        },
        "replay": {
            "budget": replay_requested,
            "budget_mode": replay_budget_mode(config),
            "fixed_total_budget": config.replay_transitions,
            "transitions_per_task": config.replay_transitions_per_task,
            "seed": config.replay_seed,
            "strict_counts": config.replay_strict_counts,
            "previous_suites": previous_suites,
            "requested_count": replay_requested,
            "selected_count": replay_selected,
            "sources": replay_sources,
        },
        "summary": {
            "manifest_path": str(manifest_path),
            "total_selected_count": current_selected + replay_selected,
            "current_selected_count": current_selected,
    ...
    if config.dry_run:
        print(f"DRY_RUN enabled; not writing replay manifest {manifest_path}")
    else:
        manifest_path.parent.mkdir(parents=True, exist_ok=True)
        with manifest_path.open("w", encoding="utf-8") as handle:
            json.dump(payload, handle, indent=2)
            handle.write("\n")
        print(f"Replay manifest saved to {manifest_path}")

    print(
        "Replay mix for "
        f"{suite}: current={current_selected}, replay={replay_selected}/{replay_requested}, "
        f"total={current_selected + replay_selected}"
    )
    return ReplayManifestResult(path=manifest_path, payload=payload)


def replay_summary_for_results(manifest_result: ReplayManifestResult) -> dict[str, Any]:
    payload = manifest_result.payload
    return {
        "manifest": str(manifest_result.path),
        "budget": payload["replay"]["budget"],
        "budget_mode": payload["replay"].get("budget_mode", "fixed_total"),
        "transitions_per_task": payload["replay"].get("transitions_per_task", 0),
        "seed": payload["replay"]["seed"],
        "current_selected_count": payload["summary"]["current_selected_count"],
        "replay_requested_count": payload["summary"]["replay_requested_count"],
        "replay_selected_count": payload["summary"]["replay_selected_count"],
        "total_selected_count": payload["summary"]["total_selected_count"],
        "sources": [
            {
                "suite": source["suite"],
            ...
            for source in payload["replay"]["sources"]
        ],
    }
...
def train_suite(
    ...
    if replay_manifest is not None:
        print(f"Replay manifest: {replay_manifest}")
...
class ReplayMixedLeRobotDataset:
    """A small adapter that lets LeRobot train from selected frames across suites."""

    def __init__(self, datasets: dict[str, Any], items: list[tuple[str, int]], meta_source_key: str) -> None:
        if not items:
            raise RuntimeError("Replay mixed dataset is empty; check TRAIN_TASK_IDS and replay settings.")

        self.datasets = datasets
        self.items = items
        self.meta_source_key = meta_source_key
        self.meta = datasets[meta_source_key].meta
        self.repo_id = "local/replay_mixed_libero"
        self.root = None
        self.episodes = None
    ...
    def num_episodes(self) -> int:
        return sum(dataset.num_episodes for dataset in self.datasets.values())
    ...
    def __getitem__(self, idx: int) -> dict[str, Any]:
        source_key, source_idx = self.items[idx]
        return self.datasets[source_key][source_idx]

    def __repr__(self) -> str:
        return (
            f"{self.__class__.__name__}("
            f"num_frames={self.num_frames}, sources={list(self.datasets)})"
        )


def build_abs_to_relative_index(dataset: Any) -> dict[int, int]:
    return {as_int(index): relative_index for relative_index, index in enumerate(dataset.hf_dataset["index"])}


def selected_current_indices(dataset: Any, task_ids: list[int]) -> list[int]:
    task_id_set = {int(task_id) for task_id in task_ids}
    indices = dataset.hf_dataset["index"]
    task_indices = dataset.hf_dataset["task_index"]
    return [
        as_int(index)
        for index, task_index in zip(indices, task_indices)
        if as_int(task_index) in task_id_set
    ]
...
def make_replay_dataset_factory(manifest_path: str | Path):
    manifest = load_replay_manifest(manifest_path)

    def make_replay_dataset(cfg: Any) -> ReplayMixedLeRobotDataset:
        import torch
        ...
        current_repo_id = current["repo_id"]
        ...
        current_meta = LeRobotDatasetMetadata(
            current_repo_id,
            root=current_root,
            revision=cfg.dataset.revision,
        )
        delta_timestamps = resolve_delta_timestamps(cfg.policy, current_meta)

        datasets: dict[str, Any] = {}
        index_maps: dict[str, dict[int, int]] = {}

        def get_dataset(source_key: str, repo_id: str, root: str | Path) -> Any:
            if source_key not in datasets:
                datasets[source_key] = LeRobotDataset(
                    repo_id,
                    root=Path(root),
                ...
                index_maps[source_key] = build_abs_to_relative_index(datasets[source_key])
            return datasets[source_key]

        current_key = f"current:{current['suite']}"
        current_dataset = get_dataset(current_key, current_repo_id, current_root)
        items: list[tuple[str, int]] = []

        for absolute_index in selected_current_indices(current_dataset, current["task_ids"]):
            items.append((current_key, index_maps[current_key][absolute_index]))

        for replay_source in manifest["replay"]["sources"]:
            if not replay_source["indices"]:
                continue
            source_key = f"replay:{replay_source['suite']}"
            get_dataset(source_key, replay_source["repo_id"], replay_source["root"])
            source_index_map = index_maps[source_key]
            for absolute_index in replay_source["indices"]:
                items.append((source_key, source_index_map[as_int(absolute_index)]))

        dataset = ReplayMixedLeRobotDataset(datasets, items, current_key)
        ...
        print(
            "Replay child dataset ready: "
            f"current={manifest['summary']['current_selected_count']}, "
            f"replay={manifest['summary']['replay_selected_count']}, "
            f"loaded_total={len(dataset)}"
        )
        return dataset
...
def import_lerobot_train_module() -> Any:
    ...
    raise ModuleNotFoundError("Could not import a LeRobot train module.")


def run_replay_train_child(manifest_path: str, train_args: list[str]) -> int:
    if not train_args:
        raise ConfigError(f"{REPLAY_CHILD_FLAG} requires lerobot-train arguments")

    train_module = import_lerobot_train_module()
    replay_make_dataset = make_replay_dataset_factory(manifest_path)

    try:
        import lerobot.datasets.factory as dataset_factory

        dataset_factory.make_dataset = replay_make_dataset
    except ModuleNotFoundError:
        pass

    train_module.make_dataset = replay_make_dataset
    sys.argv = ["lerobot-train", *train_args]
    train_func = getattr(train_module, "train", None) or getattr(train_module, "main", None)
    if train_func is None:
        raise RuntimeError("Imported LeRobot train module has neither train() nor main().")
    train_func()
    return 0
...
def run_workflow(config: Config) -> int:
    ...
    if config.run_train:
        ...
        for suite_index, suite in enumerate(train_suites):
            replay_manifest_result: ReplayManifestResult | None = None
            previous_suites = train_suites[:suite_index]
            if previous_suites and replay_enabled(config):
                replay_manifest_result = build_replay_manifest(config, suite, previous_suites)

            adapter_checkpoint = train_suite(
                config,
                suite,
                previous_policy,
                replay_manifest_result.path if replay_manifest_result is not None else None,
            )
            if config.patch_checkpoint:
            ...
            if replay_manifest_result is not None:
                train_run["replay"] = replay_summary_for_results(replay_manifest_result)
...
def main() -> int:
    if len(sys.argv) >= 2 and sys.argv[1] == REPLAY_CHILD_FLAG:
        try:
            return run_replay_train_child(sys.argv[2], sys.argv[3:])
        except IndexError:
            print(f"Usage: {Path(sys.argv[0]).name} {REPLAY_CHILD_FLAG} MANIFEST [lerobot args...]", file=sys.stderr)
            return 2
